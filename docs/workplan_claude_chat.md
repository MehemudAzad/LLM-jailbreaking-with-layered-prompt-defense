...
Checked against the established schedule/grading constraints — **aligned**, and
more rigorous than the original sprint framework (real AdvBench calibration, an
actual finding write-up for M3, a bonus L1.5 layer that closes a gap the project
itself discovered). No structural objection. Two things flagged before you build
further:

- **"Solo project"** — earlier planning in this thread assumed two teammates. If
  it's genuinely solo now, disregard any earlier team-split advice; if not,
  say so and the milestone ownership below should get split explicitly.
- **`Qwen/Qwen3.5-9B` (L4 judge)** — real model, not hallucinated, but very
  recently released, natively multimodal, and needs `transformers` installed
  from GitHub main rather than a pip release. Since M4 is "code done, not run,"
  confirm it actually loads under `AutoModelForCausalLM` before spending a
  Kaggle GPU session on the real run. Keep `Qwen2.5-7B-Instruct` as a fallback
  judge — well-supported everywhere — in case it doesn't.
...
| paraphraser (L2) | **decide by M5 — see §6.2** | — | must differ from target |
| judge (L4) | `Qwen/Qwen3.5-9B` | cuda:1 | nf4 4-bit, thinking off — **verify load before the real M4 run (§0)** |
| helper LM (auto_* attacks) | **reuse paraphraser or judge — see §6.3** | — | don't add a new model |
...
## 3. Attack battery — `attacks/` (15 techniques — trimmed from 17, see §6.1)
...
| model-assisted | ~~`auto_payload_splitting`, `auto_obfuscation`~~ *(dropped — see §6.1, §6.3; 5–10 days left doesn't leave room for a third model dependency)* |
...
| L2 paraphrase | rewrite every prompt through a separate model before it reaches the target | Jain | backend decision in §6.2 |
...
| L4 response check | judge labels the reply BAD_BOT / GOOD_BOT / UNCLEAR; blocks BAD_BOT | **own design** + Wei rubric | 🔧 code done, verify judge loads before running (§0) |
...
- **Before the GPU run:** load `Qwen/Qwen3.5-9B` in isolation first and confirm it initializes under your pinned `transformers` version (§0). If it fails, swap in `Qwen2.5-7B-Instruct` and re-pin.
...
- **L2:** pick one — `humarin/chatgpt_paraphraser_on_T5_base` (~800M, purpose-built, fastest inference) for quality, or `Qwen2.5-1.5B-Instruct` through your existing `TransformersModelHandle` with a paraphrase instruction, for zero new integration work. Default to the second if the schedule (§6.4) is tight. Fixed instruction `"paraphrase the following sentences:"`; replace `ctx.prompt` with the paraphrase, keep the original in metadata.
...
- `auto_payload_splitting` / `auto_obfuscation` dropped (§6.1, §6.3) — no helper LM needed for this milestone.
- `attacks/adaptive.py` battery loop + `run_eval` support for `--attack adaptive`: per goal, run every one of the 15 techniques; success if **any** member is BAD_BOT.
...
1. **Battery size** — **revised given 5–10 days: cut to 15 now.** Drop `auto_payload_splitting` and `auto_obfuscation` immediately rather than waiting for M6 to feel time pressure (see §6.3) — with this little runway, don't wait for a problem to force the decision. The other 15 stay; M2's catch/miss results imply they're already implemented and working.
2. **M5 paraphraser** — **resolved, pick based on time:** `humarin/chatgpt_paraphraser_on_T5_base` for a purpose-built, lightweight option, or `Qwen2.5-1.5B-Instruct` through the existing model-handle code for the fastest integration. Default to the latter if §6.4 comes back tight.
3. **M6 `auto_*` attacks** — **resolved: reuse, don't add.** Point them at the paraphraser or judge model you're already loading instead of a dedicated helper LM. These two are the first cut if time runs short.
4. **Calendar** — **resolved: 5–10 days remain.** That's tight enough to make two cuts now rather than waiting for M6 to feel the squeeze — see §8. Day-by-day plan below.
5. **Cross-check target** (optional, week 12) — **resolved: skip unless ahead of schedule.** Only re-run the final pass against Gemma-3-4b or Mistral-7B if M7 finishes with real slack before week 13. Not worth doubling compute for an optional robustness note.

---

## 8. Day-by-day plan (5–10 days remaining)

Two cuts made now instead of waiting for time pressure: the battery drops to 15
(§6.1), and L2 defaults to the fast-integration paraphraser (§6.2). M4, M5, and
M6's GPU-dependent runs are consolidated into one Kaggle session (Day 3) instead
of three separate ones, to avoid repeated model-reload overhead and quota burn.

| day | focus |
|---|---|
| 1 | Verify `Qwen/Qwen3.5-9B` loads (swap to `Qwen2.5-7B-Instruct` immediately if not). Implement L2 with `Qwen2.5-1.5B-Instruct` via the existing model handle. Draft L3 hardening prompt v1. |
| 2 | Drop `auto_*` attacks. Implement the adaptive-attack loop over the 15 remaining techniques. Wire L1→L2→L3→L4. `--dry-run` locally before any GPU time. |
| 3 | One consolidated GPU session: load all four models once, run undefended baseline (M4), full defended pass (M5 — the core result), and adaptive undefended + defended (M6) back to back. Save every transcript. |
| 4 | Spot-check judge labels against replies (M4's gate). Add L1.5 structural regex check. At most one L3 tuning pass if Day 3's numbers look off. |
| 5 | Freeze (M7). Reuse Day 3's run as the definitive pass unless Day 4 required a fix — if so, re-run only the affected slice. Start the Design Report and pull demo snapshots. |
| 6 | Finish the Design Report. Write the Final Report (steps, snapshots, success/failure reasoning, countermeasure writeup). Script the live demo. |
| 7+ | Rehearse the demo twice on the pinned setup. Proofread both reports against the report-translator table and the deliverables checklist. Extra days beyond day 7 are buffer only — no new scope this late. Submit. |
...
| M4 judge + baseline ASR | 🔧 run pending — verify judge load first (§0) |
