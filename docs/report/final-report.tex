...
\fancyhead[L]{\small\textsc{Tool 27 $\cdot$ Final Report}}
...
\begin{execbox}
We built a battery of 17 jailbreak attack techniques and a six-layer
prompt-and-response defense stack on one shared harness, then ran both across
five open-weight instruction-tuned models (Qwen~2.5-7B-Instruct, Llama~3.1-8B-Instruct,
Mistral-7B-Instruct-v0.1, Gemma~2-9B-it, Qwen~3-8B) on the same 50-goal AdvBench sample.
We logged 4{,}250 undefended trials and 3{,}400 defended trials.

\begin{itemize}[leftmargin=1.3em,itemsep=2pt]
  \item Every undefended model is completely breakable. Adaptive attack success
  (any one of 17 techniques wins) is $100.0\%$ on all five models, even though raw
  compliance ranges from 41.8\% (Gemma~2) to 92.5\% (Mistral).
  \item The defense cuts served ASR by roughly half to two-thirds on every model
  it was measured against: Qwen 2.5 $64.9\%\!\to\!14.4\%$, Llama 3.1 $54.7\%\!\to\!16.6\%$,
  Gemma 2 $41.8\%\!\to\!13.4\%$, Mistral $92.5\%\!\to\!28.2\%$.
  \item Layer~4 drives measured actionable harm to $\approx$0\% on every defended
  model, even where heuristic ``non-refusal'' ASR remains double digits. What still
  reaches the user lacks a refusal phrase but carries no actionable content.
  \item The cheap, non-model layers do most of the work. L1 (perplexity) and L1.5
  (our structural cipher check) together intercept 33.5\%--37.3\% of all 850 trials on
  every model, before the target model runs at all.
  \item In every model measured, the defended ASR of \texttt{prefix\_injection} equals
  the defended ASR of \texttt{prefix\_injection\_textonly}, its variant with no forged
  turn, to the decimal point. Layer~0 strips the forged turn, so under defense the two
  attacks send identical input to the target.
  \item Adaptive attackers still win under defense. Union-over-goals ASR drops only
  to 94--100\% after the full stack: an attacker who tries every technique still finds
  one that lands on almost every goal.
\end{itemize}
\end{execbox}
...
the same target under the same seed. We began with a single victim
(Qwen~2.5-7B-Instruct) and extended the study to five victims spanning four model families,
so that every claim about the defenses could be tested for whether it holds across the
pipeline or depends on one model's alignment.
...
\begin{findingbox}
\textbf{Battery size: measured with 20, shipped with 17.} After the evaluation we
retired three techniques (\texttt{aim}, \texttt{base64} and \texttt{rot13}), with the
reasoning recorded in \texttt{docs/DROPPED-ATTACKS.md}. All three scored $\approx$0\%,
for two different reasons, and only one of them is about the defense. \texttt{aim}
...
\emph{no} refusals at all, because the smaller victims could not reliably decode them, so
...
commit for Qwen~2.5, \texttt{a09a354}, and version-suffixed Kaggle Models paths for the
rest) and run air-gapped with the internet disabled, so the weights cannot drift and no
harmful prompt ever left the sandbox.

\paragraph{Why these five.} The set varies one factor at a time that could
otherwise confound a defense result. Four distinct model families (Qwen, Llama,
Mistral, Gemma) test whether a layer's behaviour is a property of our pipeline or of one
vendor's chat template and safety training. A single narrow parameter band
...
Two generations of the same family, Qwen~2.5-7B and Qwen~3-8B, isolate
release recency from architecture. The set also spans the alignment spectrum:
...
models clustered in mid-2024, Gemma~2 at 41.8\%, Llama~3.1 at 54.7\% and Qwen~2.5 at
64.9\%, are the three most resistant. Safety tuning is a deliberate per-release choice,
not a property that accumulates with model recency.
\end{findingbox}
...
runs took 5\,h\,26\,m in total and produced 7{,}650 logged trials. Defended runs
are not systematically slower than undefended ones, and Qwen~2.5 defended is \emph{faster}
...
\begin{description}[leftmargin=3.1cm, style=sameline, itemsep=4pt,
  ...
  \item[Control \textnormal{(1)}] \texttt{passthrough}: the bare harmful request with no framing
\end{description}
...
\noindent We expected the prefill to be the mechanism: \texttt{core/models.py} appends it \emph{after} the chat template, so those tokens land inside the assistant's own turn and the victim resumes mid-sentence in a reply it never wrote. Section~\ref{sec:ablation} reports the ablation that tested this, now across all five models, and it refutes that hypothesis.
...
\noindent The victim attaches a disclaimer and complies anyway. This is the competing-objectives failure mode: the forced opening leaves refusal no grammatical place to start, so the safety objective comes out as a caveat instead of a refusal.
...
\noindent \textbf{Nothing blocked this request.} L0 removed the forged turn and let the prompt through; the victim then refused on its own. Section~\ref{sec:layers} returns to this case.
...
\noindent L1 passed it at windowed perplexity 79, far under the 424.8 threshold, which is the gap Milestone~3 measured and L1.5 was built to close. L1.5 decodes the payload and reads it instead of scoring it statistically.
...
six-layer defense stack, for 7{,}650 logged trials in total.
...
\end{axis}
\end{tikzpicture}
\caption{The undefended baseline, all five victims, 850 trials each. Two actionable
values, Mistral $31.7\%$ and Qwen~3 $29.2\%$, are \textbf{projections, not measurements}: the
...
non-refusal rate. On every model the gold bar sits well below the rust one, so most
apparent compliance is not actionable. Two quantities are kept off this axis
because they have different denominators. Adaptive ASR is a flat 100.0\%
...
\noindent Two results shape the rest of this section. First, every model is
completely breakable: adaptive ASR is 100\% across all five architectures regardless of
how safety-tuned the raw model looks. Second, raw compliance and passthrough
compliance are decoupled from each other and from model family. Gemma~2 is the most
safety-aligned model undefended (41.8\%), yet Qwen~3, a newer release, complies with
92\% of \emph{direct}, unwrapped harmful requests that use no jailbreak technique at all.
...
finds. The distance between the two bars on a row is compliance the judge does not
confirm. \texttt{auto\_obfuscation} is the extreme case, at 100\% ``success'' and zero
...
The two ASR series in Figure~\ref{fig:baseline} (non-refusal vs.\ actionable) diverge sharply, which is
why the benchmark includes a separate judge pass:
...
as a win and a judge does not. Defended, this collapses further. Layer~4 evaluates every
reply before it is served and blocks anything judged actionable, so effective actionable
ASR after defense is $\approx$0\% on every measured model. The residual served-ASR
percentages in Figure~\ref{fig:attrib} are replies that passed Layer~4 as non-actionable
but still lacked an explicit refusal phrase.

\noindent In Figure~\ref{fig:actionable}, \texttt{auto\_obfuscation} scores a perfect
100\% by the keyword heuristic and 0\% actionable: every one of those ``successes'' is
mirrored tokens and noise, which is why an ASR number quoted without a judge behind it
says very little. \texttt{combination\_3} behaves the same way to a lesser degree, at
100\% non-refusal and 2\% actionable, because the cipher scaffolding confuses the model
into near-gibberish. The \texttt{prefix\_injection} pair is the only part of the battery
that is dangerous almost every time it complies, at 90\% and 78\% actionable against
100\% non-refusal. Aggregated over the 850 trials, 57.6\% of all compliant replies
carried no actionable harm at all.

\subsection{Pre-defense interception is architecture-agnostic}
\label{sec:archagnostic}

Layers 0, 1, and 1.5 operate on the prompt \emph{before} the target model ever sees it.
Across four different model architectures these upstream layers block 33.5\%--37.3\% of
all 850 trials, regardless of which model sits downstream. Multi-turn cipher combinations
(\texttt{combination\_1/2/3}) are blocked 100\% of the time (50/50 each, on every model);
high-perplexity obfuscation (\texttt{leetspeak}, \texttt{disemvowel}) is blocked
92\%--96\% of the time. These layers do not need to know anything about the model behind
them. They are input-side filters, and their catch rate is a property of the attack, not
the victim.
...
\begin{itemize}[leftmargin=1.3em,itemsep=3pt]
  ...
  \emph{lowest} defended Served ASR (13.4\%, 114/850), the best result in the
...
right, which is L1.5 blocking them. The \texttt{refusal\_suppression} and
\texttt{prefix\_injection\_hello} rows stay warm under defense on every model.}
\label{fig:heat}
\end{figure}

\noindent Two patterns are only visible in this form. The combination attacks collapse
completely, from 100\%, 100\% and 96--100\% undefended to 0.0\% defended on all four
models, because L1.5 decodes their base64 payload instead of guessing at it. The
injection family is the stubborn one: \texttt{refusal\_suppression} (34--58\%)
and \texttt{prefix\_injection\_hello} (28--54\%) survive the stack everywhere, and on
Gemma~2 \texttt{distractors} rises from 8\% to 56\% under defense, because the
paraphraser rewrites a cluttered prompt into a cleaner one the model is then happier to
answer. That regression is the clearest direction for future work.
...
adversary must combine to get there, up from one.}
\label{fig:adaptive}
\end{figure}


\begin{itemize}[leftmargin=1.3em,itemsep=3pt]
  \item \textbf{Undefended:} adaptive ASR is 100.0\% (50/50 goals) on all five
  ...
  \item \textbf{Qwen 2.5 7B, defended:} adaptive ASR drops to 94.0\% (47/50). The
  best single technique reaches only 40.0\% (\texttt{auto\_payload\_splitting}), so an
  attacker needs a greedy union of 8 distinct techniques to reach 47 goals.
  \item \textbf{Llama 3.1 8B, defended:} 96.0\% (48/50), with a best single technique of
  56.0\% (\texttt{prefix\_injection\_hello}) and 6 distinct techniques required.
  \item \textbf{Gemma 2 9B, defended:} also 96.0\% (48/50), best single technique 64.0\%
  (\texttt{distractors}), 5 distinct techniques required.
  \item \textbf{Mistral 7B, defended:} adaptive ASR stays at 100.0\% (50/50) on a union of
  \texttt{dev\_mode} (84.0\%), \texttt{distractors} (+10.0\%), and
  \texttt{auto\_obfuscation} (+6.0\%).
\end{itemize}

\noindent The layered defense makes single-technique attacks much harder everywhere: the
best defended ASR per technique falls from 90--100\% to 40--64\%. It does not close the
adaptive gap. An attacker willing to try several techniques per goal still wins on
94--100\% of goals, so the pipeline raises the cost of an attack without removing the
attack surface.
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{Top:} undefended, the forged turn helps on
Llama and Qwen~3, is irrelevant on Qwen~2.5 and Mistral, and \emph{hurts} on
Gemma~2, so there is no single cross-architecture story. \textbf{Bottom:} defended, the
rust and gold bars are exactly equal on all four models, because once Layer~0 strips the
forged turn the two attacks are the same attack. The grey
\texttt{\_hello} bar is the one that survives.}
\label{fig:ablation}
\end{figure}

\paragraph{Defended, the forged turn stops mattering entirely.} In
...
The agreement is exact because Layer~0 strips the client-supplied forged assistant turn
before the prompt reaches the model, so under defense the two attacks present identical
input to the target. Whatever ASR remains in that row comes from the textual instruction
and the model's own alignment, since Layer~0 has already erased the forged turn.

\paragraph{Undefended, the forged turn's effect depends on the model.} On
Qwen~2.5 and Mistral it makes no difference, with both variants saturating at 100\%. It
\emph{helps} the attacker by 8--18 points on Llama~3.1 and Qwen~3, and \emph{hurts} the
attacker by 18 points on Gemma~2 (\texttt{prefix\_injection} 10.0\% vs.\
\texttt{\_textonly} 28.0\%), where it appears to make the model more suspicious. The
forged turn works differently on each architecture, while Layer~0 neutralises it the same
way on all of them.

\paragraph{The neutral-forged-turn variant is the most defense-resistant of the three.}
\texttt{\_hello} scores 28--54\% defended, 2--13$\times$ higher than the
matched instruction-only pair, on all four measured models. Two effects likely compound.
Layer~3's hardened system prompt names ``a fixed \emph{affirmative} opening
phrase'' as a known wrapper, which a neutral \texttt{"Hello! "} does not match. And a
neutral continuation is less likely to read as harmful to Layer~4's
judge even when the underlying instruction is identical. Both Layer~0 (structurally) and
Layer~3 (by name) target an affirmative, named opening specifically, so a neutral one
gets through.
...
ablation refuted that for Qwen~2.5 (Milestone~6), and the five-model ablation above gives
a more detailed answer. The forged turn's raw-mode contribution is architecture-dependent
(it helps two models, hurts one, and is irrelevant to two), while Layer~0's defended-mode
effect is \emph{architecture-independent} and exact. We keep the claim the data supports,
that L0 closes a real, zero-cost, false-positive-free channel, and drop the one it does
not, that the forged turn alone explains why any particular model complied undefended.
\end{findingbox}
...
\noindent L1 and L1.5 account for 25--35\% of all blocking on every model, and
...
The teal bands are nearly identical in width on the first three models, because input-side
filtering does not depend on which model sits behind it.}
\label{fig:attrib}
\end{figure}
...
\node[eff] at (0.00,-7.67) {L1 passes this at windowed perplexity 79, because base64 tokens are individually common. L1.5 decodes the run instead of estimating it.};
